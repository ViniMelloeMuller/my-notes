...
Um sistema é um conjunto de objetos, conhecidos como componentes desse sistema, que atuam juntos como um todo para atingir um determinado objetivo. É uma abstração, i.e, são construidos pela mente humana para dar propósito aos componentes.
\end{definition}

Sistemas possuem as seguintes propriedades: 1. Os obejtivos de um sistema podem ser claramente bem definidos; 2. Sistemas são atomísticos, i.e, eles podem ser separados em componentes que interagem direta ou indiretamente; e 3. Recursividade: sistemas são divisíveis em subsistemas, que por si só são sistemas.

Sistemas possuem \textbf{Estados}, que são propriedades que caracterizam o sistema em qualquer momento. Enquanto o \textbf{Ambiente} é o conjunto de componentes que não fazem parte do sistema porem seu comportamento pode afetá-lo diretamente.
...
A engenharia de segurança de sistemas começa no momento em que as perdas forem definidas e os perigos relacionados, identificados. A \textbf{Análise de Perigos} envolve a identificação de cenários que descrevem condições nas quais o sistema passa a ser inseguro. Perigos são totalmente removidos quando:

\begin{enumerate}
	\item Eliminando os estados perigosos da operação do sistema;
	\item Eliminando as perdas associadas com aquele estado;
\end{enumerate}
...
Um design ou projeto de um sistema deve seguir uma hierarquia de definições:

\begin{enumerate}
	\item \textbf{Objetivos}: A razão da construção do sistema;
	\item \textbf{Requisitos}: Os obejtivos específicos em termos de declarações concretas e verificáveis;
	\item \textbf{Restrições}: As maneiras aceitáveis em que os objetivos e requisitos podem ser atingidos.
\end{enumerate}
